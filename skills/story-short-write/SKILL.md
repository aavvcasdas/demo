...
3. **每句话必须有用**。不推动剧情、不铺垫反转、不推高情绪的句子 → 删。**严禁元叙事说教**（如“我没有像三流爽文那样大喊大叫”、“我是一个极度理智的人”），用动作和决策本身（如：直接点OA离职、直接甩出全款房本）来展现人设。
4. **开头 3 句定生死，结尾定传播**。开头必须包含倒叙钩子（极简交代最大冲突与反转），把“最大的屈辱”和“最核心的底牌”在同一场景引爆。结尾必须有余韵。
5. **逻辑无 Bug 护航**。行动必须符合现实社会规律（如：辞职需要交接、不上班需要合理的收入掩护、租房不能送人押金）。避免“强行降智”或“为了打脸而打脸”的脱节剧情。
6. **默认第一人称**。短篇网文（盐言/七猫短篇等）绝大多数用第一人称，代入感最强。除非题材明确需要第三人称（如多视角悬疑），否则一律用「我」。
