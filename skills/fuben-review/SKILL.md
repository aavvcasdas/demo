...
3. **功能 / 制度存在性**：正文里每一个 App 功能（访客记录 / 已读 / 撤回 / 定位）和制度动作（举报加分 / 转账退回 / 面试回避）各搜一次确认现实存在；不存在 → **S1**（67 v2「朋友圈访客」）。
4. 搜索结果只进报告，不进正文。
...
| 共鸣（感同身受） | 核心行动是观众做过 / 被做过的事；评论区会出现「这不就是我」 | S1 |
| 报应一一对应 | 设定 报应表 ≥5 行，正文每行都能找到「做」与「还」两个行号 | S2 |
