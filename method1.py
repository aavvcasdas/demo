...
def main():
    ...
    test_dataset = OcularDataset(test_paths, val_labels, vit_transform)
    ...
    num_workers = max(1, num_cores - 1)  # 保留一个核心给系统
    print(f"使用 {num_workers} 个CPU核心进行数据加载")
    ...
    train_loader = DataLoader(
        train_dataset, 
        batch_size=batch_size,  # 降低批次大小以适应CPU
        shuffle=True, 
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            if counter >= patience:
                print(f"早停：验证损失连续{patience}次未改善")
                break
...
class OcularDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 如果路径以 '../../archive' 开头，调整为相对于当前目录的路径
        if img_path.startswith('../../archive'):
            img_path = img_path.replace('../../archive', './archive')
        
        try:
            image = Image.open(img_path).convert('RGB')
        except FileNotFoundError:
            print(f"无法找到文件: {img_path}")
            # 使用一个替代图像或生成一个空白图像
...
def train_epoch(
    ...
    for batch_index, (inputs, labels) in enumerate(train_pbar):
        # 数据转移至设备
...
def validate_epoch(
    ...
    with torch.no_grad():
        ...
        for inputs, labels in val_pbar:
            ...
            probs = torch.sigmoid(outputs)
            preds = (probs > 0.5).float()
            all_preds.append(preds.numpy())  # 省略.cpu()，因为已经在CPU上
